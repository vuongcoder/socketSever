...
/// Những dòng code ở trên phần này là phần cài đặt, các bạn hãy đọc thêm về angularjs để hiểu, cái này không nhảy cóc được nha!
    ...
    $scope.CamBienNhietDo = "Không biết nữa ahihi, chưa thấy có thằng nào cập nhập hết";
    $scope.leds_status = [1, 1]
	
	////Khu 2 -- Cài đặt các sự kiện khi tương tác với người dùng
	//các sự kiện ng-click, nhấn nút
	$scope.updateSensor  = function() {
	    mySocket.emit("SENSORS")
	}
	...
	mySocket.on('SENSORS', function(json) {
	//$scope.CamBienNhietDo = (json.Temperature == 1) ? "Hot :))" : "Cool rồi yeah ahihi"
	...
	mySocket.on('connect', function() {
		...
		mySocket.emit("SENSORS") //Cập nhập trạng thái mưa
	})
